...
export default function ChildcategoryEditPage({ childcategory, onNavigate }) {
  const [name, setName] = useState(childcategory?.name || '');
  const [subcategoryId, setSubcategoryId] = useState(childcategory?.subcategoryId || '');
  const [status, setStatus] = useState(childcategory?.status === 'Active' || childcategory?.status === 'active');
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      await childcategoryService.update(childcategory.Id, { name: name.trim(), subcategoryId: subcategoryId || null, status: status ? 'Active' : 'Inactive' });
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-bold text-gray-800">Childcategory Edit</h1>
      ...
      <div className="max-w-4xl bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
        ...
        <div>
          ...
          <input type="text" defaultValue={childcategory?.metaTitle || ''} className={inputCls} />
        </div>
