...
const NAV_LINKS = ['NEW ARRIVALS', 'ATTAR & OILS', 'PERFUMES', 'SPECIAL OFFERS', 'BY BRAND'];
...
const MOCK_ORDERS_CUSTOMER = [
  { id: 1, invoice: '000001', date: '29-11-2025', amount: 2441,  status: 'Delivered' },
  { id: 2, invoice: '000027', date: '29-11-2025', amount: 770,   status: 'Confirmed' },
  { id: 3, invoice: '000058', date: '11-12-2025', amount: 1258,  status: 'Pending'   },
  { id: 4, invoice: '000100', date: '07-02-2026', amount: 1429,  status: 'Pending'   },
  { id: 5, invoice: '000290', date: '21-04-2026', amount: 360,   status: 'Delivered' },
];
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col" style={{ background: '#f5f5f5' }}>
      ...
      <div className="bg-yellow-50 border-b border-yellow-200 px-4 py-1.5 flex items-center gap-3">
        <span className="text-xs text-yellow-800 font-medium">⚠ Viewing as customer: <strong>{name}</strong></span>
        <button
          type="button"
          onClick={onBack}
      ...
      <header className="bg-black text-white">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <div className="flex items-center gap-5">
            ...
            <button className="flex flex-col items-center gap-0.5 text-white hover:text-yellow-300 relative transition">
              ...
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">1</span>
            </button>
          </div>
        </div>
        ...
        <nav className="border-t border-gray-800">
          <div className="max-w-6xl mx-auto px-4 flex items-center gap-6 py-2">
            {NAV_LINKS.map((link) => (
              <button key={link} className="flex items-center gap-1 text-xs text-gray-300 hover:text-white transition whitespace-nowrap">
                {link}
                {(link === 'ATTAR & OILS' || link === 'PERFUMES' || link === 'BY BRAND') && <ChevronDown size={11} />}
              </button>
            ))}
          </div>
        </nav>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-6 flex gap-5 items-start">
        ...
        <div className="flex-1 bg-white rounded-lg shadow-sm overflow-hidden">
          {activeTab === 'account' && (
            <AccountTab name={name} phone={phone} email={email} address={address} />
          )}
          {activeTab === 'orders' && <OrdersTab />}
          {activeTab === 'edit' && <EditTab name={name} phone={phone} email={email} address={address} />}
      ...
      <footer className="bg-gray-900 text-gray-300 mt-auto">
        <div className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-4 gap-6">
          ...
          <div>
            ...
            {['Refund & Return Policy', 'Terms & Conditions', 'Privacy Policy', 'How To order'].map((l) => (
              <p key={l} className="text-xs text-gray-400 hover:text-white cursor-pointer mb-1.5">{l}</p>
            ))}
          </div>

          {/* Customer Links */}
          <div>
            <p className="text-white font-semibold text-sm mb-3">CUSTOMER LINK</p>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4">
              {['f', 'in', 'yt', 'wa', 'm'].map((s) => (
                <button key={s} className="w-7 h-7 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold flex items-center justify-center transition">
                  {s}
                </button>
              ))}
            </div>
            <p className="text-xs text-gray-400 font-semibold mb-1">DELIVERY PARTNER</p>
            <div className="flex gap-2">
              <span className="text-xs bg-gray-700 px-2 py-1 rounded text-gray-300">SteadFast</span>
              <span className="text-xs bg-gray-700 px-2 py-1 rounded text-gray-300">Pathao</span>
            </div>
          </div>
        </div>
        <div className="border-t border-gray-800 text-center py-2">
          <p className="text-xs text-gray-500">© Wazih <span className="text-blue-400">Websolution IT</span></p>
        </div>
      </footer>
    </div>
  );
          ...
          {[
            ...
            <tr key={label} className="border-b border-gray-100">
              ...
              <td className="py-3 text-gray-800">
                {label === 'Image' ? (
                  <div className="w-14 h-14 rounded-full bg-gray-100 border border-dashed border-gray-300 flex items-center justify-center text-2xl">👤</div>
                ) : value}
              </td>
            </tr>
          ))}
          ...
          <tbody>
            {MOCK_ORDERS_CUSTOMER.map((o, i) => (
              <tr key={o.id} className="border-b border-gray-50 hover:bg-gray-50/60 transition">
                ...
                <td className="px-3 py-2.5 font-mono text-gray-700">{o.invoice}</td>
                <td className="px-3 py-2.5 text-gray-500">{o.date}</td>
                <td className="px-3 py-2.5 font-medium text-gray-800">৳{o.amount.toLocaleString()}</td>
  ...
  return (
    <div className="p-6">
      ...
      <form className="space-y-4 max-w-lg" onSubmit={(e) => e.preventDefault()}>
        {[['name','Name'],['phone','Phone'],['email','Email'],['address','Address']].map(([key, label]) => (
          <div key={key}>
        ...
        <button type="submit" className="bg-teal-500 hover:bg-teal-600 text-white text-sm font-semibold px-5 py-2 rounded-lg transition">
          Save Changes
        </button>
      </form>
    </div>
  ...
  return (
    <div className="p-6">
      ...
      <form className="space-y-4 max-w-lg" onSubmit={(e) => e.preventDefault()}>
        ...
        <button type="submit" className="bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-semibold px-5 py-2 rounded-lg transition">
          Update Password
        </button>
      </form>
    </div>
