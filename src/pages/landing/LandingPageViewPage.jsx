...
const PAYMENT_LOGOS = ['VISA', 'bKash', 'Nagad', 'Rocket', 'UPAY', 'Tap', 'DBBL', 'QCash', 'UnionPay', 'MTB'];
...
export default function LandingPageViewPage({ campaign, onBack }) {
  ...
  const title = campaign?.campaignTitle || campaign?.title || 'Attar & Perfume Combo Pack';
  const subTitle = campaign?.subTitle || 'অল্প সময়ে ঘরে বসে প্রিমিয়াম সুগন্ধির কালেকশন পান';
  ...
  const descriptionTitle = campaign?.descriptionTitle || 'এই ক্যাম্পেইনের বিশেষ অফার';
  ...
  const deliveryCharge = SHIPPING_OPTIONS.find((option) => option.id === form.shipping)?.charge || 0;
  const total = price + deliveryCharge;

  useEffect(() => {
    let active = true;
    landingPageService.getAll({ limit: 100 })
      .then((res) => {
        if (!active) return;
  ...
  const relatedProducts = useMemo(
    () => landingPages
      ...
      .map((page) => ({
        ...
        image: page.bannerImageUrl || heroImage,
      })),
  ...
  async function handlePlaceOrder() {
    ...
    try {
      const shippingLabel = SHIPPING_OPTIONS.find((option) => option.id === form.shipping)?.label || 'Inside Dhaka';
  ...
  const template = normalizeTemplate(campaign?.template || campaign?.campaignTemplate);
  const viewData = {
    form,
    set,
    productName,
  ...
  if (showTrackOrder) {
    return <TrackOrderPage onBack={() => setShowTrackOrder(false)} />;
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#fbfaf6] text-slate-900">
      ...
      >
        ...
        <main className="mx-auto max-w-[840px] px-4 pb-12">
          ...
          <Notice tone="yellow">
            আজকে কিনুন ৫ পিস আতর পারফিউম কম্বো প্যাকের সাথে নিশ্চিত ২ টি পুরস্কার
            <br />
            <span className="text-emerald-700">পণ্যটিতে থাকছে খুবই সাশ্রয়ী দামে সুগন্ধির কালেকশন</span>
          </Notice>
          <Notice tone="purple">
          ...
          <section className="mt-5 overflow-hidden rounded border border-amber-100 bg-[#24170d] shadow-sm">
            <img src={bannerImage} alt={productName} className="h-auto w-full object-cover" />
          </section>

          <SectionBadge>এই ক্যাম্পেইনের পুরস্কার</SectionBadge>

          <section className="overflow-hidden rounded border border-slate-200 bg-sky-100 shadow-sm">
            {prizeImageSource ? (
              <img src={prizeImageSource} alt="Campaign prize" className="h-auto w-full object-cover" />
            ) : (
              <div className="grid min-h-[300px] items-center gap-4 bg-gradient-to-br from-sky-300 via-white to-slate-200 p-8 md:grid-cols-[0.85fr_1fr]">
                <div className="rounded-2xl bg-gradient-to-br from-orange-500 to-red-700 p-5 text-center text-white shadow-xl">
                  ...
                  <p className="rounded-full bg-white px-4 py-2 text-sm font-black text-red-600">স্বপ্নপূরণ ক্যাম্পেইন</p>
                  <p className="mt-4 text-xs">প্রতি অর্ডারে থাকছে পুরস্কার জেতার সুযোগ</p>
                </div>
                <div className="relative min-h-[210px]">
                  ...
                  <div className="absolute right-10 top-4 rounded-full bg-white px-5 py-3 text-center text-xl font-black text-sky-700 shadow-lg">
                    স্বপ্নপূরণ
                    <span className="block text-sm text-red-500">ক্যাম্পেইন</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <section className="mt-4 rounded border border-orange-100 bg-white/90 p-5 shadow-sm">
          ...
          <div className="mx-auto mt-5 max-w-xs rounded border border-emerald-200 bg-gradient-to-r from-rose-50 to-emerald-50 p-4 text-center shadow-sm">
            ...
            <p className="mt-1 text-lg font-black text-emerald-700">অফার মূল্য - {formatMoney(price)} টাকা</p>
          </div>

          <section id="order-now" className="mt-8">
            <div className="text-center">
              <h2 className="text-2xl font-black text-slate-900">Complete Your Order</h2>
              <p className="mt-1 text-xs text-slate-500">Fill in your details to place your order</p>
            </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-[1.25fr_0.75fr]">
              <div className="space-y-4">
                <CheckoutPanel title="Customer Information" icon={<User size={15} />}>
                  <div className="grid gap-3 md:grid-cols-2">
                    <Input label="Full Name *" value={form.name} onChange={(value) => set('name', value)} placeholder="Enter your full name" />
                    <Input
                      label="Phone Number *"
                      value={form.phone}
              ...
              <aside className="space-y-4">
                <div className="rounded border border-blue-200 bg-blue-50 p-4">
                  <h3 className="text-xs font-bold text-blue-700">Campaign Product</h3>
                  <div className="mt-3 flex items-center gap-3 rounded border border-emerald-300 bg-white p-3">
                    <img src={bannerImage} alt={productName} className="h-12 w-16 rounded object-cover" />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-black text-slate-800">{productName}</p>
                      <p className="text-xs text-slate-500">Special campaign offer</p>
                    </div>
                    <p className="text-sm font-black text-emerald-700">{formatMoney(price)}</p>
                  </div>
                </div>
                ...
                <div className="rounded border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-3 space-y-2 text-sm">
                    ...
                    <SummaryRow label="Delivery" value={`BDT ${formatMoney(deliveryCharge)}`} />
                    <div className="flex gap-2 pt-1">
                      ...
                      />
                      <button type="button" className="rounded bg-emerald-100 px-3 text-xs font-bold text-emerald-700">
                        Apply
                      </button>
                    </div>
                    <SummaryRow label="Total" value={`BDT ${formatMoney(total)}`} strong />
                    <SummaryRow label="Pay Now" value="BDT 0.00" />
                    <SummaryRow label="Due on Delivery" value={`BDT ${formatMoney(total)}`} strong highlight />
                  </div>
                </div>
                ...
                <div className="rounded border border-amber-100 bg-amber-50 p-4">
                  ...
                  {orderError ? <p className="mt-3 rounded bg-red-50 px-3 py-2 text-xs font-bold text-red-500">{orderError}</p> : null}
                  <button
                    type="button"
          ...
          <section className="mt-10 text-center">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              {relatedProducts.map((item) => (
                <ProductCard key={item.name} item={item} image={bannerImage} />
              ))}
            </div>
          </section>

          {shortDescription ? (
            <section className="mt-8 rounded border border-slate-200 bg-white/80 p-5 text-sm leading-7 text-slate-600">
...
function PreviewShell({ title, onBack, tone = 'light', children }) {
  return (
    <div className={`flex-1 overflow-y-auto ${tone === 'dark' ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
...
function OrderSuccessPage({ order, form, productName, productImage, price, deliveryCharge, total, onContinue, onTrack }) {
    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 text-slate-900">
      <main className="mx-auto max-w-3xl px-4 py-10">
        <div className="text-center">
          ...
          <h1 className="mt-4 text-3xl font-black text-slate-900">Order Placed Successfully!</h1>
          <p className="mt-2 text-sm text-slate-500">Thank you for your order. We've received your request and will process it shortly.</p>
        </div>
        ...
        <section className="mt-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="p-5">
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                <h3 className="text-sm font-black text-slate-900">Order Information</h3>
                <InfoRow label="Order Number:" value={orderNumber} />
                <InfoRow label="Order Date:" value={orderDate.toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })} />
                <InfoRow label="Payment Status:" value="Unpaid" danger />
              </div>
              <div>
                <h3 className="text-sm font-black text-slate-900">Delivery Information</h3>
                <p className="mt-3 text-xs text-slate-600">Contact: <span className="font-bold text-slate-900">{form.name}</span></p>
                <p className="text-xs text-slate-600">{form.phone}</p>
                <p className="mt-2 text-xs text-slate-600">Address: <span className="font-bold text-slate-900">{form.address}</span></p>
                <p className="mt-2 text-xs text-slate-600">Delivery Zone: <span className="font-bold text-slate-900">{order.customerArea || 'Inside Dhaka'}</span></p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center gap-4 rounded-lg bg-slate-50 p-4">
              <img src={productImage || heroImage} alt={productName} className="h-16 w-16 rounded object-cover" />
              <div className="min-w-0 flex-1">
              ...
              <div className="text-right text-sm font-black text-slate-900">
                ...
                <p className="text-xs text-blue-600">BDT {formatMoney(price)}</p>
              </div>
            </div>

            <div className="mt-5 space-y-3 border-t border-slate-200 pt-5 text-sm">
              <InfoRow label="Delivery Charge:" value={`BDT ${formatMoney(deliveryCharge)}`} />
        ...
        <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <Step number="1" title="Order Confirmation" text="You will receive a confirmation call within 2-4 hours to verify your order details." />
          <Step number="2" title="Processing" text="Once confirmed, your order will be processed and prepared for delivery." />
          <Step number="3" title="Delivery" text="Your order will be delivered to your address within 2-5 business days." />
        </section>

        <div className="mt-6 text-center">
          <button type="button" onClick={onContinue} className="rounded bg-blue-600 px-5 py-2 text-sm font-bold text-white">
            Back to Landing Page
          </button>
          <button type="button" onClick={onTrack} className="ml-3 rounded bg-slate-800 px-5 py-2 text-sm font-bold text-white">
            Track Order
          </button>
        </div>
      </main>
...
function InfoRow({ label, value, danger }) {
  return (
    <div className="mt-3 flex items-center justify-between gap-4 text-xs">
      ...
      <span className={`text-right font-bold ${danger ? 'text-orange-600' : 'text-slate-900'}`}>{value}</span>
    </div>
  );
}
...
function Step({ number, title, text }) {
  return (
    <div className="mt-5 flex gap-3">
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-black text-blue-600">{number}</span>
      <div>
    ...
    <div className="flex-1 overflow-y-auto bg-[#f4f0e8] text-slate-900">
      ...
      >
        <div className="mx-auto max-w-6xl">
          <button type="button" onClick={onBack} className="mb-5 inline-flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-blue-600">
            <ArrowLeft size={14} />
            Back
          </button>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-lg font-semibold text-slate-500">Monitor your order journey from placement to delivery with real-time updates</p>
            </div>
          </div>

          <section className="mt-6 rounded-xl border border-white/80 bg-white/85 p-8 text-center shadow-sm backdrop-blur">
            <h2 className="text-2xl font-black text-slate-900">Search Your Order</h2>
            <p className="mt-2 text-sm font-semibold text-slate-500">Enter your phone number to find all your orders</p>
            <div className="mx-auto mt-7 flex max-w-2xl flex-col gap-3 sm:flex-row">
              <label className="relative min-w-0 flex-1">
                <Phone size={22} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                <input
                  value={phone}
                  onChange={(event) => setPhone(event.target.value)}
                  onKeyDown={(event) => { if (event.key === 'Enter') searchOrders(); }}
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase text-slate-400">Order Number</p>
          <h3 className="mt-1 text-xl font-black text-slate-900">{order.orderId}</h3>
          <p className="mt-2 text-sm font-semibold text-slate-600">{order.productName}</p>
        </div>
        <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-black capitalize text-blue-700">{status}</span>
      </div>
      ...
      <main className="mx-auto max-w-6xl px-4 pb-14">
        <section className="grid min-h-[520px] items-center gap-8 py-8 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="overflow-hidden rounded border border-emerald-100 bg-white shadow-sm">
            <img src={bannerImage} alt={productName} className="h-[460px] w-full object-cover" />
          </div>
          <div>
            <span className="inline-flex rounded bg-emerald-100 px-3 py-1 text-xs font-black uppercase tracking-wide text-emerald-700">
              Premium Campaign
            </span>
            <h1 className="mt-4 text-4xl font-black leading-tight text-slate-950 md:text-5xl">{title}</h1>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-3">
              {['Authentic Product', 'Fast Delivery', 'Cash on Delivery'].map((item) => (
                <div key={item} className="rounded border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700 shadow-sm">
                  {item}
                </div>
              ))}
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <PrizeShowcase prizeImageSource={prizeImageSource} productName={productName} />
        ...
        <RelatedProductsSection relatedProducts={relatedProducts} bannerImage={bannerImage} />
      </main>
      <Footer phone={phone} />
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="border-b border-white/10 bg-slate-900">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 text-xs font-bold text-slate-300">
          ...
          <button type="button" onClick={onTrackOrder} className="text-amber-300 hover:text-amber-200">Track Order</button>
          <span>{phone}</span>
        </div>
      </div>
      ...
      <main className="mx-auto max-w-6xl px-4 pb-14">
        <section className="grid gap-6 py-8 lg:grid-cols-[1fr_420px]">
          <div className="overflow-hidden rounded border border-white/10 bg-white text-slate-950">
            <div className="relative min-h-[430px]">
              <img src={bannerImage} alt={productName} className="absolute inset-0 h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-r from-slate-950/80 via-slate-950/30 to-transparent" />
              <div className="relative z-10 max-w-2xl px-6 py-10 text-white md:px-10">
                <span className="rounded bg-red-500 px-3 py-1 text-xs font-black uppercase">Limited Offer</span>
          ...
          <div id="order-now" className="rounded border border-emerald-400/40 bg-white p-4 text-slate-950 shadow-2xl">
            <h2 className="text-xl font-black">Quick Order</h2>
            <p className="mt-1 text-xs font-semibold text-slate-500">Order summary and customer information</p>
            <div className="mt-4">
        ...
        <section className="grid gap-5 md:grid-cols-3">
          ...
          <div className="rounded border border-white/10 bg-white/5 p-5">
            ...
            <p className="mt-2 text-2xl font-black">{formatMoney(price)} টাকা</p>
          </div>
          <div className="rounded border border-white/10 bg-white/5 p-5">
            <h3 className="text-sm font-black text-emerald-300">Regular Price</h3>
            <p className="mt-2 text-2xl font-black line-through opacity-70">{formatMoney(originalPrice)} টাকা</p>
          </div>
        </section>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
          ...
          <PrizeShowcase prizeImageSource={prizeImageSource} productName={productName} dark />
        </section>
...
function PricePill({ price, originalPrice, dark }) {
  return (
    <div className={`rounded px-5 py-3 shadow-sm ${dark ? 'bg-white text-slate-950' : 'bg-slate-950 text-white'}`}>
      <p className="text-xs font-bold opacity-70">
        Regular <span className="line-through">{formatMoney(originalPrice)} টাকা</span>
      </p>
...
function PrizeShowcase({ prizeImageSource, productName, dark }) {
  return (
    <section className={`overflow-hidden rounded border shadow-sm ${dark ? 'border-white/10 bg-white/10' : 'border-slate-200 bg-white'}`}>
      <div className="border-b border-slate-200 px-5 py-3">
        <h2 className={`text-lg font-black ${dark ? 'text-white' : 'text-slate-900'}`}>Campaign Prize</h2>
      </div>
      {prizeImageSource ? (
        <img src={prizeImageSource} alt={`${productName} prize`} className="h-80 w-full object-cover" />
      ) : (
        <div className="grid min-h-[320px] place-items-center bg-gradient-to-br from-cyan-100 via-white to-amber-100 p-8 text-center text-slate-900">
          <div>
            <p className="text-sm font-black uppercase text-red-600">Reward Campaign</p>
            <p className="mt-2 text-4xl font-black">Special Gift</p>
            <p className="mt-2 text-sm font-semibold text-slate-600">প্রতি অর্ডারে থাকছে আকর্ষণীয় পুরস্কারের সুযোগ</p>
          </div>
        </div>
      )}
    </section>
  ...
  return (
    <section className="rounded border border-slate-200 bg-white p-5 text-slate-900 shadow-sm">
      ...
      <p className="mt-3 text-sm leading-7 text-slate-600">
        {text || 'এই ক্যাম্পেইনে প্রিমিয়াম পণ্য, সহজ অর্ডার, দ্রুত ডেলিভারি এবং ক্যাশ অন ডেলিভারি সুবিধা আছে।'}
      </p>
    </section>
  );
}
...
function WinnerGrid({ reviewImages, title, dark }) {
  return (
    <section className={`mt-8 rounded border p-5 shadow-sm ${dark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-white'}`}>
      <h2 className={`text-center text-2xl font-black ${dark ? 'text-white' : 'text-slate-900'}`}>{title}</h2>
      <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
...
function OrderFormBlock({ data, compact }) {
  const { form, set, productName, price, bannerImage, deliveryCharge, total, placingOrder, orderError, onPlaceOrder } = data;

  return (
    <section id={compact ? undefined : 'order-now'} className={compact ? '' : 'mt-8'}>
      {!compact ? (
        <div className="text-center">
          <h2 className="text-2xl font-black text-slate-900">Complete Your Order</h2>
          <p className="mt-1 text-xs text-slate-500">Fill in your details to place your order</p>
      ...
      <div className={`mt-5 grid gap-4 ${compact ? 'grid-cols-1' : 'md:grid-cols-[1.25fr_0.75fr]'}`}>
        <div className="space-y-4">
          <CheckoutPanel title="Customer Information" icon={<User size={15} />}>
...
function RelatedProductsSection({ relatedProducts, bannerImage }) {
  if (relatedProducts.length === 0) return null;

  return (
    <section className="mt-10 text-center">
      <h2 className="text-lg font-black text-slate-900">You Might Also Like</h2>
      <p className="mt-1 text-xs text-slate-500">More landing page campaigns</p>
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {relatedProducts.map((item) => (
          <ProductCard key={item.id} item={item} image={item.image || bannerImage} />
        ))}
      </div>
    </section>
  );
...
function TopStrip({ phone, onTrack }) {
  return (
    <div className="bg-[#1d1d1b] text-white">
      <div className="mx-auto flex max-w-[840px] items-center justify-between px-4 py-2 text-[11px]">
        <p>Need any help? Call {phone} or contact support</p>
        <div className="flex items-center gap-3 text-amber-300">
          <button type="button" onClick={onTrack} className="font-bold hover:text-amber-100">Track your order</button>
...
function Notice({ children, tone }) {
  ...
  return (
    <div className={`mx-auto mt-2 max-w-[680px] rounded border px-4 py-3 text-center text-base font-black leading-7 ${classes}`}>
      {children}
    </div>
  );
}
...
function JumpButton({ children, targetId }) {
  return (
    <button
      ...
      className="inline-flex items-center gap-2 rounded bg-emerald-600 px-5 py-2 text-sm font-black text-white shadow-sm transition hover:bg-emerald-700"
    >
...
function SectionBadge({ children }) {
  return (
    <div className="my-4 text-center">
      <span className="inline-flex rounded border border-blue-200 bg-white px-6 py-3 text-xl font-black text-red-600 shadow-sm">
        {children}
      </span>
    </div>
  );
...
function WinnerCard({ name, index, image }) {
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="relative h-28 bg-gradient-to-br from-rose-200 via-amber-100 to-sky-200">
        {image ? (
        ...
        <div className="absolute bottom-2 right-3 rounded-full bg-amber-400 px-3 py-1 text-[10px] font-black text-white shadow">
          #{index + 1}
        </div>
      </div>
      <div className="p-2 text-center">
        <p className="truncate text-xs font-black text-red-600">{name}</p>
        <p className="text-[10px] font-bold text-slate-500">R15 বাইক বিজয়ী</p>
      </div>
    </div>
  );
}
    ...
    >
      <span className="flex items-center justify-between gap-3">
        <span className="inline-flex items-center gap-2 text-sm font-bold text-slate-800">
          <span className={`h-2.5 w-2.5 rounded-full ${selected ? 'bg-orange-500' : 'bg-slate-300'}`} />
          {label}
        </span>
        {price ? <span className="text-xs font-black text-blue-600">{price}</span> : null}
      </span>
      {note ? <span className="mt-1 block pl-4 text-xs text-slate-500">{note}</span> : null}
    </button>
  );
}
...
function SummaryRow({ label, value, strong, highlight }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-2 last:border-b-0">
      <span className={`${strong ? 'font-black text-slate-900' : 'text-slate-600'}`}>{label}</span>
      <span className={`${strong ? 'font-black' : 'font-semibold'} ${highlight ? 'text-emerald-700' : 'text-slate-800'}`}>{value}</span>
    </div>
  );
}
...
function ProductCard({ item, image }) {
  ...
  return (
    <div className="overflow-hidden rounded border border-slate-200 bg-white text-left shadow-sm">
      <div className="relative h-36 bg-gradient-to-br from-amber-100 via-white to-emerald-100">
        <img src={image || heroImage} alt={item.name} className="h-full w-full object-cover opacity-80" />
        <span className="absolute right-2 top-2 rounded bg-white px-2 py-1 text-[10px] font-black text-red-600 shadow">
          40% Off
        </span>
      </div>
      <div className="p-3">
        <h3 className="line-clamp-2 min-h-10 text-sm font-black text-slate-800">{item.name}</h3>
        ...
        <div className="mt-2 flex items-center gap-2">
          <span className="text-base font-black text-red-600">{formatMoney(item.price)}</span>
...
function Footer({ phone }) {
  return (
    <footer className="bg-[#222321] text-slate-200">
      <div className="mx-auto grid max-w-[840px] gap-8 px-4 py-9 md:grid-cols-[1.3fr_0.8fr_0.8fr_1fr]">
        <div>
          <div className="mb-3 flex items-center gap-2 text-sm font-black text-white">
            <span className="flex h-7 w-7 items-center justify-center rounded bg-amber-500 text-xs">W</span>
            ওয়াজিহ
          </div>
          <p className="text-sm font-black">{phone}</p>
          <p className="mt-3 text-xs leading-6 text-slate-400">Corporate office: Dhaka, Bangladesh</p>
        </div>
        <FooterLinks title="Quick Links" links={['Customer Support', 'All Products', 'Categories', 'Track My Order']} />
        <FooterLinks title="Follow Us" links={['Facebook', 'YouTube', 'TikTok', 'Instagram']} />
        <div>
          <h3 className="text-sm font-black text-white">Important Links</h3>
          <button type="button" className="mt-3 rounded bg-slate-700 px-4 py-2 text-xs text-slate-300">
            Refund Policy
          </button>
        </div>
      </div>
      <div className="mx-auto max-w-[840px] px-4 pb-7">
        <div className="flex flex-wrap items-center justify-center gap-2 rounded bg-white p-4">
          {PAYMENT_LOGOS.map((logo) => (
            <span key={logo} className="rounded border border-slate-200 px-2 py-1 text-[10px] font-black text-slate-600">
              {logo}
            </span>
          ))}
          <CheckCircle2 size={18} className="text-emerald-500" />
        </div>
        <p className="mt-5 text-center text-[11px] text-slate-500">© Wazih 2026. Powered by Websolution IT</p>
      </div>
    </footer>
  );
}
...
function getProductName(campaign) {
  ...
  return campaign.product?.name || campaign.productName || 'Attar & Perfume Combo Pack';
}

function normalizeTemplate(value) {
  const template = String(value || 'Template Design 1').trim();
  if (template === 'Template Design 2' || template === 'Template Design 3') return template;
