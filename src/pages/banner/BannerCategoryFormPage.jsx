...
export default function BannerCategoryFormPage({ mode = 'create', category, onSave, onNavigate }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-100 p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h1 className="text-lg font-bold text-gray-800">Banner Category {isEdit ? 'Edit' : 'Create'}</h1>
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => onNavigate('banner_category')} className="rounded-full bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700">
            Manage
          </button>
        </div>
      </div>
