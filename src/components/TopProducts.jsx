  ...
  return (
    <div className="bg-white rounded-xl shadow overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3" style={{ background: 'linear-gradient(90deg, #1d4ed8, #3b82f6)' }}>
        <span className="text-white font-semibold text-sm">Top Selling Products</span>
          ...
          {loading
            ? Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="border-b border-gray-50 animate-pulse">
                  ...
                  <td className="px-4 py-3 text-right"><div className="h-3 w-8 bg-gray-200 rounded ml-auto" /></td>
              ...
              : topProducts.map((p, i) => (
                <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  <td className="px-4 py-3">
                    {p.productImage
                      ? <img src={`${BASE_URL}/${p.productImage}`} alt={p.productName} className="w-10 h-10 object-cover rounded border border-gray-200" onError={(e) => { e.target.style.display='none'; e.target.nextSibling.style.display='flex'; }} />
                      : null
                    }
                    <div className={`w-10 h-10 bg-gray-100 rounded flex items-center justify-center text-gray-300 text-xs border border-gray-200 ${p.productImage ? 'hidden' : 'flex'}`}>IMG</div>
                  </td>
                  <td className="px-4 py-3 text-gray-700 text-xs max-w-[160px] truncate">{p.productName}</td>
                  <td className="px-4 py-3 text-right text-gray-600 text-xs">{fmt(p.qty)}</td>
                  <td className="px-4 py-3 text-right text-gray-800 font-medium text-xs">৳ {fmt(p.total)}</td>
                </tr>
              ))
          }
        </tbody>
